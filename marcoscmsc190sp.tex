...
two users' compatibility. Each criteria in the questionnaire is worth a certain amount of points. The criteria as as
follows:

    \subsubsection{Cleanliness}
        The cleanliness criteria is determined by 2 fields from each user:
        \begin{itemize}
        ...
        $$magnetism_1 = 10-user1's cleanliness - user2's preference$$
        and
        $$magnetism_2 = 10-user2's cleanliness - user1's preference$$
    ...
    \subsubsection{Smoker}
        ...
        user2's preference to a smoking partner is equal to user1's smoking profile, another 5 is added. This criteria gives a
    ...
    \subsubsection{Start Date}
        ...
        available when user1 wishes to move in. If this is the case, this criteria will be given a score of 10.
        Otherwise, it returns a score of 0.

    \subsubsection{Rent Cost}
        The Rent Cost is a criteria used to determine whether or not a room is within a user's budget.
        The price range of user1's input is compared to user2's input for the rent cost. The variables that are
        considered are:
        \begin{itemize}
            \item rent\textunderscore start(user1) - User1's self-rating of their cleanliness level.
            \item rent\textunderscore end(user1) - User1's preferred cleanliness level for their potential roommate.
            \item rent\textunderscore cost(user2) - User2's monthly rent bill.
        \end{itemize}
        If the monthly rent of the room is within the range set by user1, then a score of 10 is given to this criteria.
    ...
    \subsubsection{Travel Time to UPLB}
        This criteria is filled out by the user as the number of minutes it takes for a user to reach UPLB.


This scoring algorithm will be applied to the cartesian product of the users that have a room and the users that are looking for
a room.
